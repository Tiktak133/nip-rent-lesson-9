...
def display_apartments(manager):
    ...
    for apartment in manager.apartments.values():
        ...
        apartment_bills = [
            bill for bill in manager.bills if bill.apartment == apartment.key
        ]
        if apartment_bills:
            ...
            for bill in apartment_bills:
                ...
                print(
                    f"      • {bill.type:<15} {format_currency(bill.amount_pln):>15}  Due: {bill.date_due}  Period: {month_year}",
                )
...
def display_tenants(manager):
    ...
    for tenant in manager.tenants.values():
        ...
        print(
            f"   Agreement: {tenant.date_agreement_from} to {tenant.date_agreement_to}",
        )

        # Find transfers for this tenant
        tenant_transfers = [
            transfer for transfer in manager.transfers if transfer.tenant == tenant.name
        ]
        if tenant_transfers:
            ...
            for transfer in tenant_transfers:
                ...
                print(
                    f"      • {format_currency(transfer.amount_pln):>15}  Date: {transfer.date}  Period: {month_year}",
                )
...
def display_monthly_settlement(manager, apartment_key: str, year: int, month: int):
    ...
    print_section_header(
        f"MONTHLY SETTLEMENT  —  {apartment.name} ({apartment_key})  |  {month:02d}/{year}",
    )
    ...
    if apartment_bills:
        for bill in apartment_bills:
            print(
                f"      • {bill.type:<20} {format_currency(bill.amount_pln):>15}  Due: {bill.date_due}",
            )
    else:
    ...
    if all_apt_transfers:
        for tr in all_apt_transfers:
            print(
                f"      • {tr.tenant:<25} {format_currency(tr.amount_pln):>15}  Date: {tr.date}",
            )
    else:
